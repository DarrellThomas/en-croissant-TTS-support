...
| W-004 | AI-assisted database encoding/decoding | Explore using a small AI model as an encoding/decoding agent in the database layer. Current bottleneck is legal-move-index encoding requiring full position replay. Possible directions: learned move prediction for better compression, AI-assisted position hashing, or neural move encoding that eliminates `legal_moves()` calls. Needs research — unclear how yet, but the opportunity is real. |
