...
CREATE INDEX IF NOT EXISTS games_date_time_idx ON Games(Date DESC, UTCTime DESC, ID DESC);
...
CREATE INDEX IF NOT EXISTS games_white_elo_date_time_idx
    ON Games(WhiteElo, Date DESC, UTCTime DESC, ID DESC);
CREATE INDEX IF NOT EXISTS games_black_elo_date_time_idx
    ON Games(BlackElo, Date DESC, UTCTime DESC, ID DESC);
