...
async getDbGame(file: string, gameId: number) : Promise<Result<NormalizedGame | null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_db_game", { file, gameId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
